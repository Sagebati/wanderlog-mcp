...
async function getOrCreateContext(cookieRaw) {
    ...
    const promise = (async () => {
        const config = {
            ...
            userAgent: process.env.WANDERLOG_USER_AGENT ??
        ...
        const ctx = { config, rest, pool, tripCache, userId: user.id, authenticated: true };
    ...
    const handleMcp = async (req, res) => {
        ...
        if (!cookie) {
            res.status(401).json({
                ...
                error: {
                    code: -32000,
                    message: "Missing Authorization header. Set Bearer token to your Wanderlog connect.sid cookie value.",
                },
                id: null,
            });
            return;
...
main().catch((err) => {
    console.error(`[wanderdog] fatal: ${err.stack ?? err}`);
    process.exit(1);
});
