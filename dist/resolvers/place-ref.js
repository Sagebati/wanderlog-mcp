...
export function parseOrdinal(ref) {
    ...
    if (wordMatch) {
        ...
        if (n !== undefined)
            return { position: n, rest: wordMatch[2].trim() };
    }
    return null;
}
/**
 ...
 */
export function resolvePlaceRef(trip, ref) {
...
function findByLeftSide(trip, ref) {
...
function matchRoleKeyword(trip, ref) {
    ...
    if (TRAIN_KEYWORDS.has(ref)) {
        return firstBlockOfSectionType(trip, (s) => s.type === "transit", (b) => b.type === "train");
    }
    return [];
}
function firstBlockOfSectionType(trip, sectionPredicate, blockPredicate) {
    const sections = trip.itinerary.sections;
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
        ...
        for (let blockIndex = 0; blockIndex < section.blocks.length; blockIndex++) {
            ...
            if (blockPredicate && !blockPredicate(block))
                continue;
...
function matchPlaceName(trip, ref, mode) {
    const matches = [];
    const sections = trip.itinerary.sections;
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
        const section = sections[sectionIndex];
...
function filterByContext(trip, candidates, context) {
    if (candidates.length === 0)
        return candidates;
...
function tryResolveDay(trip, ref) {
    try {
...
function finalize(candidates) {
    ...
    if (candidates.length === 1)
        return { kind: "unique", match: candidates[0] };
    return {
        kind: "ambiguous",
...
//# sourceMappingURL=place-ref.js.map