 ...
 */
export function resolveDay(trip, ref) {
    const daySections = getDaySections(trip);
    const trimmed = ref.trim().toLowerCase();
    if (daySections.length === 0) {
        throw new WanderlogValidationError(`Trip "${trip.title}" has no day-by-day plan`, "This trip does not yet have per-day sections to target.");
    }
    ...
    if (monthDayMatch) {
        ...
        if (!month) {
            throw new WanderlogValidationError(`Unknown month "${monthName}" in day reference "${ref}"`, 'Try a format like "day 2", "May 4", or "2026-05-04".');
        }
    ...
    throw new WanderlogValidationError(`Could not understand day reference "${ref}"`, 'Try a format like "day 2", "May 4", or "2026-05-04".');
}
export function getDaySections(trip) {
    return trip.itinerary.sections.filter((s) => s.mode === "dayPlan" && s.date);
}
function pad(n) {
    return n < 10 ? `0${n}` : `${n}`;
}
function outOfRange(trip, detail) {
