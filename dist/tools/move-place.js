...
export const movePlaceInputSchema = {
    ...
    place_ref: z
        .string()
        .min(1)
        .describe("Natural-language reference to the place (or any block) to move. Same syntax as wanderlog_remove_place: partial names, ordinal prefixes ('2nd X', 'last X') and day filters ('X on day 3')."),
    before: z
        .string()
        .min(1)
        .optional()
        .describe("Place reference the block should be moved directly BEFORE. Mutually exclusive with `after`. Combine with `to_day` to disambiguate the anchor."),
    after: z
        .string()
        .min(1)
        .optional()
        .describe("Place reference the block should be moved directly AFTER. Mutually exclusive with `before`. Combine with `to_day` to disambiguate the anchor."),
    to_day: z
        .string()
        .min(1)
        .optional()
        .describe("Destination day ('day 2', 'May 4', or '2026-05-04'). Without before/after the block is appended to that day (or use `position`)."),
    position: z
        .enum(["start", "end"])
        .optional()
        .describe("Where in the destination day to place the block when no before/after anchor is given. Defaults to 'end'."),
};
...
`.trim();
export async function movePlace(ctx, args) {
    try {
        if (args.before && args.after) {
            throw new WanderlogValidationError("`before` and `after` are mutually exclusive", "Provide only one anchor.");
        }
        if (!args.before && !args.after && !args.to_day) {
            throw new WanderlogValidationError("No destination given", "Provide `before`, `after`, or `to_day`.");
        }
 ...
 */
export function buildMoveOps(fromSection, fromIndex, toSection, insertIndex, block) {
...
function resolveDestination(trip, args, source) {
    ...
    if (anchorRef) {
        ...
        if (!("match" in anchor))
            return anchor;
        if (anchor.match.sectionIndex === source.sectionIndex &&
            anchor.match.blockIndex === source.blockIndex) {
            throw new WanderlogValidationError("Anchor and moved place are the same block", "Pick a different before/after anchor.");
        }
        return {
    ...
    if (sectionIndex === -1) {
        throw new WanderlogError("Resolved day section not found in trip", "day_resolution_failed");
    }
    return {
        sectionIndex,
...
function resolveOrReport(trip, ref) {
...
//# sourceMappingURL=move-place.js.map