export function formatTripList(trips, format) {
    if (trips.length === 0)
        return "No trips found in this account.";
    if (format === "concise") {
        return trips
            .map((t) => {
    ...
    return trips
        ...
        if (t.user)
            lines.push(`Owner:    ${t.user.username}`);
        if (t.editedAt)
            lines.push(`Edited:   ${t.editedAt}`);
        return lines.join("\n");
    })
        .join("\n\n");
}
export function formatTrip(trip, format, dayFilter) {
    if (dayFilter)
        return formatDay(trip, dayFilter, format);
    const parts = [formatTripHeader(trip, format)];
...
function renderDaySection(section, format) {
...
function sectionDefaultHeading(section) {
    switch (section.type) {
        case "hotels":
            return "Hotels";
        case "flights":
...
function formatTripHeader(trip, format) {
    const dates = `${trip.startDate} → ${trip.endDate}`;
    const base = `${trip.title} · ${dates} · ${trip.days} days · ${trip.placeCount} places`;
    if (format === "concise")
        return base;
    const extras = [base, `Key: ${trip.key}`, `Privacy: ${trip.privacy}`];
    const contributorNames = trip.contributors?.map((c) => c.username).join(", ");
    if (contributorNames)
        extras.push(`Contributors: ${contributorNames}`);
    return extras.join("\n");
}
function formatDay(trip, section, format) {
    const label = formatDayLabel(section);
 ...
 */
export function formatBlockLine(block, format) {
    try {
        switch (block.type) {
            case "place":
                return formatPlaceBlock(block, format);
            case "note":
                return formatNoteBlock(block, format);
            case "checklist":
                return formatChecklistBlock(block, format);
            case "flight":
                return formatFlightBlock(block, format);
            case "train":
                return formatTrainBlock(block, format);
            default:
...
function formatPlaceBlock(block, format) {
    const p = block.place;
    if (!p?.name)
        return null;
    ...
    if (p.rating)
        parts.push(`★${p.rating} (${p.user_ratings_total ?? 0} reviews)`);
    if (p.types?.length)
        parts.push(`[${p.types.slice(0, 3).join(", ")}]`);
    if (p.formatted_address)
        parts.push(p.formatted_address);
    if (p.international_phone_number)
        parts.push(p.international_phone_number);
...
function formatNoteBlock(block, format) {
...
function formatChecklistBlock(block, format) {
    const items = block.items ?? [];
    if (items.length === 0 && !block.title)
        return null;
...
function formatFlightBlock(block, format) {
    ...
    if (block.confirmationNumber)
        parts.push(`conf. ${block.confirmationNumber}`);
    if (block.travelerNames?.length)
        parts.push(`pax: ${block.travelerNames.join(", ")}`);
    return parts.join(" · ");
}
function formatTrainBlock(block, format) {
...
function formatAirport(endpoint) {
...
function formatDayLabel(section) {
    if (!section.date)
        return "Day";
...
//# sourceMappingURL=trip-summary.js.map