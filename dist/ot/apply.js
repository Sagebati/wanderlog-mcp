...
function pathInvalid(path, reason) {
    const rendered = path.map((p) => JSON.stringify(p)).join(", ");
    throw new WanderlogError(`JSON0 op path [${rendered}] is invalid: ${reason}`, "ot_path_invalid", "Verify the op path matches the current document shape (parent containers must exist and have the correct type).");
}
function isObject(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function navigateParent(doc, path) {
    let current = doc;
 ...
 */
function applyRichTextOp(parent, key, payload) {
    const target = Array.isArray(parent)
        ? parent[key]
        : parent[key];
    // Extract current plain text from the QuillDelta
    let current = "";
    if (target && typeof target === "object" && "ops" in target) {
...
function applySingleOp(doc, op) {
    ...
    if ("si" in op || "sd" in op) {
        ...
        if ("sd" in op && op.sd !== undefined) {
            ...
            if (slice !== op.sd) {
                pathInvalid(p, `sd verification failed: expected "${op.sd}" at index ${idx}, found "${slice}"`);
            }
        ...
        if (Array.isArray(stringParent)) {
            stringParent[stringKey] = updated;
        }
        else {
            stringParent[stringKey] = updated;
        }
    ...
    if (hasLd || hasLi || hasLm) {
        ...
        if (hasLd) {
            ...
            if (op.ld !== null && JSON.stringify(existing) !== JSON.stringify(op.ld)) {
                throw new WanderlogError(`JSON0 ld verification failed at path [${p.join(", ")}]: existing element does not match provided ld value`, "ot_path_invalid", "The cached document drifted from the server snapshot. Refresh the trip before retrying.");
            }
            if (hasLi) {
                parent[last] = op.li;
            }
...
export function applyOp(doc, ops) {
